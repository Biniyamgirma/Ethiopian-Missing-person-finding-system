...
const API_BASE_URL =  'http://localhost:3004'; // Adjust if your backend URL is different

// Default image path (from public folder)
const DEFAULT_IMAGE_PATH = "/image/addis.jpg"; // As per your requirement
...
export function Header({ username }) {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      if (currentUser.firstName && currentUser.middleName) {
        setDisplayName(`${currentUser.firstName} ${currentUser.middleName}`);
  ...
  }, [currentUser,refreshInterval]); // Re-fetch when currentUser changes (e.g., after login or if policeStationId updates)
    ...
    <header className="border-b border-gray-200 bg-white px-4 py-3 dark:bg-gray-800 dark:border-gray-700">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          ...
          {currentUser && currentUser.role <= 1 && notificationCount > 0 && (
            <Button
              variant="ghost"
              size="icon"
